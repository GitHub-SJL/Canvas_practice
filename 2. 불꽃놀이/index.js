...
class Canvas extends CanvasOption {
  ...
  createTail() {
    ...
    const colorDeg = randomNumberBewtween(0, 360);
  ...
  createParticles(x, y, colorDeg) {
    ...
    for (let i = 0; i < PARTICLE_NUM; i++) {
      ...
      const _colorDeg = randomNumberBewtween(-20, 20) + colorDeg;
    ...
    const frame = () => {
      ...
      this.tails.forEach((tail, index) => {
        ...
        for (let i = 0; i < Math.round(-tail.vy * 0.5); i++) {
          const vx = randomNumberBewtween(-5, 5) * 0.05;
          const vy = randomNumberBewtween(-5, 5) * 0.05;
